...
if (chercher_form) {
    ...
    chercher_form.addEventListener('submit', async (e) => {
        ...
        btn_recherche.textContent = 'Publication en cours ...'        
        
        try {
            const response = await fetch('../scripts/chercher_une_competence.php', {
                method: 'POST',
                headers: {
            ...
            if (result.success) {
                ...
                setTimeout(()=>{
                    showAlert_recherche(result.message, true);
                },2000)

                setTimeout(()=>{
                    chercher_une_competence.classList.add('hidden');
                },3000)

                console.log(result.data);
                console.log(result.id);
            } else {
                showAlert_recherche(result.message, false);
            }
        }
        catch (err) {
            showAlert_recherche("Une erreur est survenue. Veuillez réessayer", false);
        }
...
// ALERTE DE MESSAGE
const showAlert_recherche = (message, isSuccess = true) => {

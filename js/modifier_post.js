const proposition_form = document.querySelector("#proposition_form");
const proposer_une_competence = document.querySelector("#proposer_une_competence");
const btn_proposition = document.querySelector('#btn_proposition')
const text_initial = btn_proposition.textContent;
if (proposition_form) {
    ...
    proposition_form.addEventListener('submit', async (e) => {
        e.preventDefault();
        const id = btn_proposition.dataset.post_id;
        const data = {id};
        ...
        btn_proposition.textContent = 'Modification en cours ...'

        try {
            const response = await fetch('../../scripts/modifier_post.php', {
                method: 'POST',
                headers: {
            ...
            if (result.success) {
                ...
                setTimeout(() => {
                    ...
                    btn_proposition.textContent = text_initial;
                }, 2000)

                setTimeout(() => {
                    proposer_une_competence.classList.add('hidden');
                }, 3000)
    ...
    console.error("Formulaire introuvable : #proposition_form");
}
